...
class BaseSpider(scrapy.Spider):
    ...
    def set_playwright_true(self, request, response):
        ...
        request.meta["playwright_page_methods"] = [PageMethod("wait_for_timeout", 3000),
    ...
    def parse(self, response, **kwargs):
        ...
        yield from response.follow_all(xpath=self.decryption_rules.article_url,
                                       callback=self.parse)
        # Parse pagination
        # if self.validate_pagination(response):
        #     yield from response.follow_all(xpath=self.decryption_rules.pagination,
        #                                    callback=self.parse, process_request=self.set_playwright_true)
